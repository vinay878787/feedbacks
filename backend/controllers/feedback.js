...
const admin = "follower_36yqxl57";
const { Worker } = require("worker_threads");
const path = require("path");
const Associations = require("../models/feedback");

const processFeedback = async (req, res) => {
  console.log(req.body);
  const { email, idea, name, summary, topic ,ratings} = req.body;
...
const ChokeData = async () => {
  const response = await fetch(
    "http://localhost:3000/api/v1/feedback/getfeedback"
  );
